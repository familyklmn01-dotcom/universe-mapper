...
export function normalizeData(input) {
  ...
  const relationships=relationshipSource.filter(r=>r.type!=='contains').map(r=>({id:String(r.id||createId('rel')),sourceId:String(r.sourceId||r.source||''),targetId:String(r.targetId||r.target||''),type:String(r.type||'influences'),weight:r.weight??1,strokeWidth:r.strokeWidth==null?undefined:Number(r.strokeWidth),strokeStyle:r.strokeStyle==null?undefined:String(r.strokeStyle),color:r.color==null?undefined:String(r.color),startArrow:r.startArrow==null?undefined:String(r.startArrow),endArrow:r.endArrow==null?undefined:(r.endArrow===false?false:String(r.endArrow)),manualRoute:Boolean(r.manualRoute),waypoints:Array.isArray(r.waypoints)?r.waypoints.map(point=>({x:Number(point.x)||0,y:Number(point.y)||0})):[],metadata:r.metadata&&typeof r.metadata==='object'?r.metadata:{}})).filter(r=>ids.has(r.sourceId)&&ids.has(r.targetId)&&r.sourceId!==r.targetId)
