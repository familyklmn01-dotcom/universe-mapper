...
export function normalizeData(input) {
  ...
  const nodes=rawNodes.map((n,i)=>({
    ...
    name:String(n.name||`Untitled ${i+1}`),type:String(n.type||'category'),value:n.value??'',status:String(n.status||'draft'),textAlign:['left','center','right'].includes(n.textAlign)?n.textAlign:'left',verticalAlign:['top','middle','bottom'].includes(n.verticalAlign)?n.verticalAlign:'middle',zIndex:Number.isFinite(Number(n.zIndex))?Number(n.zIndex):i,
