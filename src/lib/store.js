...
export function normalizeData(input) {
  ...
  const annotations=(Array.isArray(input.annotations)?input.annotations:[]).map((a,i)=>{const kind=annotationKinds.has(a.kind)?a.kind:(a.kind==='text'?'text':'shape');const isText=kind==='text',isFrame=kind==='frame';return {id:String(a.id||createId('annotation')),kind,shape:String(a.shape||(isText||isFrame?'rectangle':'rounded')),text:String(a.text||''),assetId:a.assetId?String(a.assetId):null,src:a.src?String(a.src):'',x:Number(a.x)||120+i*20,y:Number(a.y)||120+i*20,width:Math.max(40,Number(a.width)||180),height:Math.max(30,Number(a.height)||80),rotation:Number(a.rotation)||0,zIndex:Number.isFinite(Number(a.zIndex))?Number(a.zIndex):nodes.length+i,fontFamily:String(a.fontFamily||'Inter'),fontSize:Math.max(8,Number(a.fontSize)||18),fontWeight:String(a.fontWeight||'600'),italic:Boolean(a.italic),underline:Boolean(a.underline),align:String(a.align||(isText?'left':'center')),verticalAlign:String(a.verticalAlign||(isText?'top':'middle')),color:String(a.color||'#edf2ff'),fill:a.fill===undefined?(isText||isFrame?'transparent':'#263654'):String(a.fill),fillOpacity:Number.isFinite(Number(a.fillOpacity))?Number(a.fillOpacity):1,stroke:String(a.stroke||'#7897f5'),strokeWidth:Number.isFinite(Number(a.strokeWidth))?Math.max(0,Number(a.strokeWidth)):isText?0:2,strokeStyle:String(a.strokeStyle||'solid'),cornerRadius:Number.isFinite(Number(a.cornerRadius))?Math.max(0,Number(a.cornerRadius)):isFrame?0:10,autoFit:a.autoFit!==false,locked:Boolean(a.locked),visibility:String(a.visibility||'all'),sceneId:a.sceneId?String(a.sceneId):null,maintainAspectRatio:a.maintainAspectRatio!==false,metadata:a.metadata&&typeof a.metadata==='object'?a.metadata:{}}})
