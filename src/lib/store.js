...
export function normalizeData(input) {
  ...
  const nodes=rawNodes.map((n,i)=>({
    ...
    x:Number.isFinite(Number(n.x))?Number(n.x):120+(i%4)*250,y:Number.isFinite(Number(n.y))?Number(n.y):100+Math.floor(i/4)*130,width:Math.max(72,Number(n.width)|| (n.type==='circle'?104:184)),height:Math.max(48,Number(n.height)|| (n.type==='circle'?104:64)),locked:Boolean(n.locked),autoFit:n.autoFit!==false,textAlign:['left','center','right'].includes(n.textAlign)?n.textAlign:'left',verticalAlign:['top','middle','bottom'].includes(n.verticalAlign)?n.verticalAlign:'middle',
  ...
  const annotations=(Array.isArray(input.annotations)?input.annotations:[]).map((a,i)=>({id:String(a.id||createId('annotation')),kind:annotationKinds.has(a.kind)?a.kind:(a.kind==='text'?'text':'shape'),shape:String(a.shape||'rectangle'),text:String(a.text||''),assetId:a.assetId?String(a.assetId):null,src:a.src?String(a.src):'',x:Number(a.x)||120+i*20,y:Number(a.y)||120+i*20,width:Math.max(40,Number(a.width)||180),height:Math.max(30,Number(a.height)||80),rotation:Number(a.rotation)||0,zIndex:Number.isFinite(Number(a.zIndex))?Number(a.zIndex):nodes.length+i,fontFamily:String(a.fontFamily||'Arial'),fontSize:Math.max(8,Number(a.fontSize)||18),fontWeight:String(a.fontWeight||'600'),italic:Boolean(a.italic),underline:Boolean(a.underline),align:['left','center','right'].includes(a.align)?a.align:'left',verticalAlign:['top','middle','bottom'].includes(a.verticalAlign)?a.verticalAlign:'top',color:String(a.color||'#edf2ff'),fill:String(a.fill||((a.kind==='text'||a.kind==='frame')?'transparent':'#263654')),fillOpacity:Number.isFinite(Number(a.fillOpacity))?Number(a.fillOpacity):1,stroke:String(a.stroke||'#7897f5'),strokeWidth:Number.isFinite(Number(a.strokeWidth))?Math.max(0,Number(a.strokeWidth)):(a.kind==='text'?0:2),strokeStyle:String(a.strokeStyle||'solid'),cornerRadius:Math.max(0,Number(a.cornerRadius)||0),locked:Boolean(a.locked),visibility:String(a.visibility||'all'),sceneId:a.sceneId?String(a.sceneId):null,maintainAspectRatio:a.maintainAspectRatio!==false,autoFit:a.autoFit!==false,entityType:a.entityType||(a.kind==='frame'?'frame':a.kind),metadata:a.metadata&&typeof a.metadata==='object'?a.metadata:{}}))
