...
export function normalizeData(input) {
  ...
  const nodes=rawNodes.map((n,i)=>({
    ...
    x:Number.isFinite(Number(n.x))?Number(n.x):120+(i%4)*250,y:Number.isFinite(Number(n.y))?Number(n.y):100+Math.floor(i/4)*130,width:Math.max(72,Number(n.width)|| (n.type==='circle'?104:184)),height:Math.max(48,Number(n.height)|| (n.type==='circle'?104:64)),locked:Boolean(n.locked),autoFit:n.autoFit!==false,
